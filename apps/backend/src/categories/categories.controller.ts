...
export class CategoriesController {
	...
	@Post()
	create(@CurrentUser('userId') userId: string, @Body() dto: CreateCategoryDto): Promise<Category> {
	...
	@Get(':id')
	findOne(@Param('id') id: string, @CurrentUser('userId') userId: string): Promise<Category> {
	...
	@ApiResponse({
		...
		schema: {
			example: {
				statusCode: 409,
				message: 'Категория с именем "Кафе и рестораны" уже существует',
				error: 'Conflict',
			},
		},
	})
