...
export class TransactionsRepository {
	...
	groupByMonth(userId: string, filters: QueryTransactionsDto): Promise<MonthlyRow[]> {
		...
		if (filters.categoryIds?.length) {
			conditions.push(Prisma.sql`"categoryId" IN (${Prisma.join(filters.categoryIds)})`);
		}
		if (filters.dateFrom) {
	 ...
	 */
	private buildWhere(userId: string, filters: QueryTransactionsDto): Prisma.TransactionWhereInput {
